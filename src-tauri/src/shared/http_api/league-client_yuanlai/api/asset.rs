use crate::lcu::constant;
use crate::lcu::https::{lcu_get, lcu_get_img_as_binary};
use moka::future::Cache;
use serde::{Deserialize, Serialize};
use std::collections::HashMap;
use std::sync::{LazyLock, RwLock}; // retained only for BINARY_CACHE
...
async fn fetch_binary(url: &str) -> Result<(Vec<u8>, String), String> {
    lcu_get_img_as_binary(url).await
}
